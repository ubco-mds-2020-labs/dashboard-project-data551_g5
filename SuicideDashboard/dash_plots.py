...
def plot_suicide_gdp(data, sex, country, age, year):
    ...
    chart = alt.Chart(df, title = 'Suicide Trends per Country').mark_circle(color = 'yellow').encode(
        ...
        alt.Size('gdp_per_capita ($):Q', legend = None),tooltip = [alt.Tooltip('country:N'), alt.Tooltip('gdp_per_capita ($):Q')]).interactive().configure(background = '#282b30').configure_axis(grid = False)
    return chart.to_html()
